...
int GenerateCompleteSchemaFromClass(const FString& SchemaPath, const FString& ForwardingCodePath, int ComponentId, UClass* Class, const TArray<FString>& TypeBindingHeaders)
{
	...
	//TSharedPtr<FUnrealType> TypeInfo = BuildUnrealPropertyWrapper(Class, 0, 0);
	TSharedPtr<FUnrealReplicationDataWrapper> ReplicationDataWrapper = BuildUnrealReplicationDataWrapper(Class);
	...
	GenerateTypeBindingHeader(OutputHeader, SchemaFilename, TypeBindingFilename, Class, ReplicationDataWrapper);
	GenerateTypeBindingSource(OutputSource, SchemaFilename, TypeBindingFilename, Class, ReplicationDataWrapper, TypeBindingHeaders);
...
bool SpatialGDKGenerateInteropCode()
{
	...
	if (const FConfigSection* UserInteropCodeGenSection = GetConfigSection(ConfigFilePath, UserClassesSectionName))
	{
		...
		if (FPaths::CollapseRelativeDirectories(AbsoluteCombinedSchemaPath) && FPaths::CollapseRelativeDirectories(AbsoluteCombinedForwardingCodePath))
		{
			...
			// Copy Interop files.
			FString DiffCopyArguments = FString::Printf(TEXT("\"%s\" \"%s\""), *AbsoluteCombinedIntermediatePath, *AbsoluteCombinedForwardingCodePath);
			...
			// Copy schema files
			DiffCopyArguments = FString::Printf(TEXT("\"%s\" \"%s\""), *AbsoluteCombinedSchemaIntermediatePath, *AbsoluteCombinedSchemaPath);
